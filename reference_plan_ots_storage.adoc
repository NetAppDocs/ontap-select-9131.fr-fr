...
* Créez un pool de stockage système
Vous devez créer un pool de stockage pour les données système ONTAP Select. Vous devez connecter le pool de stockage dans le cadre de la configuration du nœud ONTAP Select.
* Connectez les disques physiques nécessaires
Les disques SSD requis doivent être connectés à l'hôte de l'hyperviseur et disponibles pour la machine virtuelle ONTAP Select. Ces disques détiennent la racine et les agrégats de données. Vous devez connecter les disques de stockage dans le cadre de la configuration du noeud ONTAP Select.
